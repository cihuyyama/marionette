"""Temp-mail provider for Blackbox signup OTP reception.

Backend: self-hosted dreamhunter2333/cloudflare_temp_email Worker (standalone
copy of grok_farm/mail_provider.py — farm packages are intentionally decoupled).
The worker is catch-all on the bound domain, but we still pre-create the
address via the admin API to obtain a per-address JWT for scoped polling.
...
OTP rule (ported from refs/novabox/providers/tempmail.py): Blackbox codes are
plain 6 digits and live in the message BODY only. The mailbox list headers
(id, date, message-id timestamps) contain their own 6-digit runs that would
cause false matches — never scan the subject.
"""
...
def extract_otp(subject: str, body: str) -> str | None:
    ...
    ``subject`` is accepted for signature compatibility with
    TempMailSession.extract_otp but deliberately ignored: message-id
    timestamps and date strings in subjects false-positive on \\d{6}.
    """
    for m in _OTP_PATTERN.finditer(body or ""):
        return m.group(1)
    return None
...
class TempMailSession:
    ...
    run inside an executor from async code."""
    ...
    def poll_otp(self, timeout: int = 120, interval: float = 3.0) -> str | None:
