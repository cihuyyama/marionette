...
def load_proxy_pool(cfg: Config) -> list[str]:
    ...
    if getattr(cfg, "no_proxy", False):
        return pool
    file_env = (cfg.proxy_file or os.getenv("QODER_PROXY_FILE") or "").strip()
    paths: list[Path] = []
    if file_env:
    ...
    inline = (os.getenv("QODER_PROXY_POOL") or "").strip()
    if inline:
...
async def launch_camoufox(cfg: Config, prog: Progress) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            f"camoufox import failed: {exc}. pip install -r requirements.txt && python -m camoufox fetch"
        ) from exc

    humanize: Any = False
    if cfg.humanize:
        humanize = 0.8 if cfg.headless else 0.5
